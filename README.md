...
4. The backend returns calculated `geometry` and a ready-to-download `dxf` string.
...
keys belong only in the backend environment, never in frontend variables. DXF
entities are generated by the backend; the frontend only downloads the returned
file content.
